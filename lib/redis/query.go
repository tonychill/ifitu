...
func (s SortingKey) Serialize() Args {
	args := Args{s.Field}
	if s.Ascending {
		args = args.Add("ASC")
	} else {
...
// NewQuery creates a new query for a given index with the given search term.
// For currently the index parameter is ignored
// TODO: make this method private since we're passing a global.Query to the search
// method now
func NewQuery(query *global.Query) *Query {
	raw, err := getRawQuery(query)
	if err != nil {
		log.Error().Err(err).Msg("error getting raw query")
		return nil
	}
	return &Query{
		Raw:     raw,
...
func NewQueryOld(raw string) *Query {
	return &Query{
		Raw:     raw,
		Filters: []Filter{},
		Paging:  Paging{DefaultOffset, DefaultNum},
...
func (q Query) serialize() Args {

	args := Args{q.Raw}.AddFlat(q.Paging.serialize())
...
func appendNumArgs(num float64, exclude bool, args Args) Args {
	if math.IsInf(num, 1) {
...
func (q *Query) SetIndex(index string) *Query {
	q.index = index
	return q
}
